import { SpriteControl } from "../../modules/vortex.js";
import { checkOutsideBounds } from "../../helpers/checkOutsideBounds.js"
import { RenderService } from "../../modules/vortex.js";
import { Entity, Vector2, InputControl } from "../../modules/vortex.js";

SpriteControl.Load("player", "src/assets/img/lemonman.png");

export const player = new Entity({
    position: new Vector2(50, 50),
    size: new Vector2(30, 30), 
    color: "#FF0000",
    speed: 4,
    // speedModifiers: [],
    direction: new Vector2(0, 0),
});

setTimeout(() => {
    SpriteControl.Attach("player", player);
}, 100)

export const playerMovement = () => {
    player.direction = new Vector2(0, 0);

    const leaveDirection = checkOutsideBounds(player, new Vector2(RenderService.vw, RenderService.vh));

    if (InputControl.IsPressed("d") && leaveDirection.x !== 1) {
        player.direction = player.direction.Add(new Vector2(1, 0));
    }

    if (InputControl.IsPressed("a") && leaveDirection.x !== -1) {
        player.direction = player.direction.Add(new Vector2(-1, 0));
    }

    if (InputControl.IsPressed("w") && leaveDirection.y !== -1) {
        player.direction = player.direction.Add(new Vector2(0, -1));
    }

    if (InputControl.IsPressed("s") && leaveDirection.y !== 1) {
        player.direction = player.direction.Add(new Vector2(0, 1));
    }

    if (!player.direction.Equals(new Vector2(0, 0))) {
        player.direction = player.direction.Normalize();
    }
};
